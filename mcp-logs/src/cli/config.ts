 ...
 */
async function handleGitignoreForLocalConfig(
  noGitignore: boolean,
  autoYes: boolean,
): Promise<void> {
  ...
  const shouldAdd = await promptYesNo(
    `   Add ${configFilename} to .gitignore?`,
    true,
  );
...
async function initConfig(args: string[]) {
  ...
  if (!isMinimal) {
    console.log(
      '💡 Tip: Lines starting with "_" are comments explaining each option',
    );
...
function showConfig() {
  ...
  console.log(
    `  2. Local config: ${getLocalConfigPath()} ${existsSync(getLocalConfigPath()) ? "✓" : "✗"}`,
  );
  console.log(
    `  3. Global config: ${getGlobalConfigPath()} ${existsSync(getGlobalConfigPath()) ? "✓" : "✗"}`,
  );
...
function setConfigValue(args: string[]) {
  ...
  if (keyIndex === -1 || keyIndex + 1 >= args.length) {
    ...
    console.log(
      "Usage: bun run index.ts config set <section.field> <value> [--global]",
    );
...
function parseValue(section: string, field: string, value: string): any {
  ...
  if (section === "storage" && field === "storage_type") {
    if (!["memory", "sqlite", "postgresql"].includes(value)) {
      throw new Error(
        `Invalid storage_type '${value}'. Must be: memory, sqlite, postgresql`,
      );
    }
  }
  ...
  if (section === "logging" && field === "log_level") {
    if (!["debug", "info", "warn", "error"].includes(value)) {
      throw new Error(
        `Invalid log_level '${value}'. Must be: debug, info, warn, error`,
      );
    }
  }
...
function listConfigKeys() {
  ...
  console.log(
    "  storage_type          - Storage backend (memory, sqlite, postgresql)",
  );
  console.log(
    "  sqlite_path           - SQLite database path (if using sqlite)",
  );
  console.log(
    "  postgres_url          - PostgreSQL connection URL (if using postgresql)",
  );
  console.log();
  console.log("📝 [logging]");
  console.log(
...
function showHelp() {
  ...
  console.log(
    "  init [options]                       Create configuration file",
  );
  console.log(
    "    --global, -g                       Create in ~/.config/mcp-logs/",
  );
  console.log("    --minimal, -m                      Create without comments");
  console.log(
    "    --yes, -y                          Auto-add to .gitignore without prompting",
  );
  console.log(
    "    --no-gitignore, -n                 Skip .gitignore management",
  );
  console.log(
  ...
  console.log(
    "  bun run index.ts config init               Create local config (prompt for .gitignore)",
  );
  console.log(
    "  bun run index.ts config init --yes         Create and auto-add to .gitignore",
  );
  console.log(
    "  bun run index.ts config init -n            Create without .gitignore",
  );
  console.log(
    "  bun run index.ts config show               Show merged config",
  );
